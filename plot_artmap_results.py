...
def fix_mem_bits(df):
    ...
    # Fuzzy ARTMAP binary
    m = df["model"].eq("FuzzyARTMAP_binary")
    df.loc[m, "memory_bits"] = (
        df.loc[m, "n_clusters"]
    ...
    # Fuzzy ARTMAP continuous
    m = df["model"].eq("FuzzyARTMAP_continuous")
    df.loc[m, "memory_bits"] = (
    ...
    # Binary Fuzzy ARTMAP
    m = df["model"].eq("BinaryFuzzyARTMAP")
    mem_bits_a = (
...
def plot_accuracy_vs_fit_time(df, n_bits_left=1, n_bits_right=16):
    """
    2x4 grid (4 datasets):
...
def plot_accuracy_vs_memory_bits(df, n_bits_left=1, n_bits_right=16):
    ...
    Within each panel:
      ...
      points only (no connecting lines).
    """
    required = {"dataset", "model", "variant", "rho", "n_bits", "accuracy_mean", "memory_bits_mean"}
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"df is missing required columns: {sorted(missing)}")
